...
# constraints
	subject to Long_Links {k in 1..psd}: sum{i in 1..l_n, t in 1..tsd} alpha[t]*x[i] <= W_n[k];
	subject to Short_Links {j in 1..e_m}: sum{i in 1..l_n, t in 1..tsd} alpha[t]*x[i] <= W_e[j];
	subject to Traffic_Demands {k in 1..psd}: sum{i in 1..l_n, t in 1..tsd} alpha[t]*x[i] >= T_d[k]; 
